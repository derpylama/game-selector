...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    connectToServer: (username) => ipcRenderer.send('connect-to-server', username),
    ...
    progressOverlay: (callback) => {
        ...
        ipcRenderer.on('progress-overlay', (event, status) => callback(status));
    },
    progressOverlayComplete: (callback) => {
        ipcRenderer.removeAllListeners('progress-overlay-complete');
        ipcRenderer.on('progress-overlay-complete', callback);
    },
});